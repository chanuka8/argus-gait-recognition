...
| **Appearance Feature Extractor** | `intelligence/appearance_embedding.py` | Local PyTorch weights (`models/model_store/weights/osnet_x0_25.pth`) | **NOT REQUIRED** | Pure local ReID CNN inference (OSNet 512D) | Yes (Inference interval) | N/A | **VERIFIED** |
| **Local Vector Gallery** | `storage/vector_store.py` | Local `.npy` / `.json` in `models/galleries/live_gallery/` | **NOT REQUIRED** | Real-time cosine similarity gallery search | Yes (< 5ms search) | N/A | **VERIFIED** |
| **Biometric Embedding Store** | `storage/firebase_embedding_store.py` | Firestore `biometric_embeddings` + `data/runtime/firebase_offline_store.json` | **REQUIRED** | Long-term cloud persistence, cross-node synchronization, provenance, lineage, and disaster recovery | No (Asynchronous / Buffered) | HIGH | **VERIFIED** |
| **Subject Enrollment** | `services/gait_service.py` -> `EmbeddingDatabase` | Local Person JSON (`data/runtime/embedding_db/persons/`) + Firebase | **REQUIRED** | Multi-node subject synchronization, identity management, and cloud backup | No (Enrollment event) | HIGH | **VERIFIED** |
...
| **Date-Aware Learning Scheduler** | `intelligence/date_aware_learning_scheduler.py` | Local `data/runtime/learning_jobs.json` + Firestore `learning_jobs` | **OPTIONAL** | Cross-cluster continual learning coordination | No (Scheduled batch) | MEDIUM | **VERIFIED** |
...
| **Continual Learning Audit Trail** | `intelligence/continual_learning_audit_trail.py` | Local `data/runtime/continual_learning_audit_trail.json` + Firestore `audit_logs` | **REQUIRED** | Tamper-evident forensic audit trail of retraining, metrics, and promotions | No (Post-training) | HIGH | **VERIFIED** |
