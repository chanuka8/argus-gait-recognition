...
class LearnedSilhouetteSegmenter:
    def __init__(self, model_path: str = "ml_platform/models/model_store/weights/silhouette_segmenter.onnx", threshold: float = 0.5) -> None:
    ...
    def _init_session(self) -> None:
        ...
        if self.model_path.exists():
            ...
            defaults = [
                ...
                Path("ml_platform/models/model_store/engines/silhouette_segmenter.onnx"),
            ]
            if str(self.model_path) in {
                "ml_platform/models/model_store/weights/silhouette_segmenter.onnx",
                "ml_platform/models/model_store/engines/silhouette_segmenter.onnx",
            }:
                for p in defaults:
                    if p.exists():
                        target_path = p
        ...
        try:
            ...
            from app.security_layer.model_integrity import ROLE_SILHOUETTE_SEGMENTER

            logical_name = "silhouette_segmenter.onnx" if target_path.name.endswith(".enc") else target_path.name
...
class SilhouetteStep:
    def __init__(
        ...
        method: str = "auto",
        model_path: str = "ml_platform/models/model_store/weights/silhouette_segmenter.onnx",
        threshold: float = 0.5,
        ...
        default_path = sil_cfg.get("model_path", "ml_platform/models/model_store/weights/silhouette_segmenter.onnx")
        self.model_path = model_path if model_path != "ml_platform/models/model_store/weights/silhouette_segmenter.onnx" else default_path
