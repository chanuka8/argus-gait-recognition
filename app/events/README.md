...
- Boundaries: Does not write events to disk directly (handled by `app/utils/event_logger.py` and `app/security_layer/security_logger.py`).
...
Pipeline Step / Alert → `app/events/event_bus.py` → `app/events/dispatcher.py` → Registered Listener Handlers.
