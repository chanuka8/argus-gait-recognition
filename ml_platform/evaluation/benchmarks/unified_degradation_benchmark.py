...
def run_unified_degradation_benchmark():
    ...
    osnet_backbone = OSNetBackbone(model_path="ml_platform/models/model_store/weights/osnet_x0_25.pth", device=device)
    ...
    fusion_unprotected = DualModalFusion(
        default_gait_weight=0.30, default_reid_weight=0.70, enabled=True, high_risk_confusion_groups=[]
    )
    fusion_safeguarded = DualModalFusion(
    ...
    for i in range(N):
        ...
        dec_c_unprot = fusion_unprotected.decide_identity(
            gal_lbl[best_g_c], g_sims_c[best_g_c], gal_lbl[best_a_c], a_sims_c[best_a_c], 0.89, 0.72
        )
        dec_c_safeg = fusion_safeguarded.decide_identity(
            gal_lbl[best_g_c], g_sims_c[best_g_c], gal_lbl[best_a_c], a_sims_c[best_a_c], 0.89, 0.72
        )
        ...
        dec_d_unprot = fusion_unprotected.decide_identity(
            gal_lbl[best_g_d], g_sims_d[best_g_d], gal_lbl[best_a_d], a_sims_d[best_a_d], 0.89, 0.72
        )
        dec_d_safeg = fusion_safeguarded.decide_identity(
            gal_lbl[best_g_d], g_sims_d[best_g_d], gal_lbl[best_a_d], a_sims_d[best_a_d], 0.89, 0.72
        )
    ...
    print(
        f"  - Gait Cosine Similarity       : Clean = {np.mean(gait_clean_sims_same):.4f} --> Degraded = {np.mean(gait_deg_sims_same):.4f} (Delta = {np.mean(gait_deg_sims_same) - np.mean(gait_clean_sims_same):.4f})"
    )
    print(
        f"  - Appearance Cosine Similarity : Clean = {np.mean(app_clean_sims_same):.4f} --> Degraded = {np.mean(app_deg_sims_same):.4f} (Delta = {np.mean(app_deg_sims_same) - np.mean(app_clean_sims_same):.4f})"
    )
    ...
    print(
        f"{'Condition & Pipeline':<35} | {'Subgroup':<15} | {'CONFIRMED':<18} | {'REVIEW_REQUIRED':<18} | {'UNKNOWN (Lost)'}"
    )
    print("-" * 110)
    print(
        f"{'Clean Probe (Unprotected)':<35} | {'Safe (N=5)':<15} | {single_clean_unprot['SAFE']['CONFIRMED']:>2} / {n_safe} ({single_clean_unprot['SAFE']['CONFIRMED'] / n_safe * 100:5.1f}%)   | {single_clean_unprot['SAFE']['REVIEW_REQUIRED']:>2} / {n_safe} ({single_clean_unprot['SAFE']['REVIEW_REQUIRED'] / n_safe * 100:5.1f}%)   | {single_clean_unprot['SAFE']['UNKNOWN']:>2} / {n_safe} ({single_clean_unprot['SAFE']['UNKNOWN'] / n_safe * 100:5.1f}%)"
    )
    print(
        f"{'':<35} | {'Confusion (N=32)':<15} | {single_clean_unprot['CONFUSION']['CONFIRMED']:>2} / {n_conf} ({single_clean_unprot['CONFUSION']['CONFIRMED'] / n_conf * 100:5.1f}%)   | {single_clean_unprot['CONFUSION']['REVIEW_REQUIRED']:>2} / {n_conf} ({single_clean_unprot['CONFUSION']['REVIEW_REQUIRED'] / n_conf * 100:5.1f}%)   | {single_clean_unprot['CONFUSION']['UNKNOWN']:>2} / {n_conf} ({single_clean_unprot['CONFUSION']['UNKNOWN'] / n_conf * 100:5.1f}%)"
    )
    print("-" * 110)
    print(
        f"{'Clean Probe (Safeguarded 5N)':<35} | {'Safe (N=5)':<15} | {single_clean_safeg['SAFE']['CONFIRMED']:>2} / {n_safe} ({single_clean_safeg['SAFE']['CONFIRMED'] / n_safe * 100:5.1f}%)   | {single_clean_safeg['SAFE']['REVIEW_REQUIRED']:>2} / {n_safe} ({single_clean_safeg['SAFE']['REVIEW_REQUIRED'] / n_safe * 100:5.1f}%)   | {single_clean_safeg['SAFE']['UNKNOWN']:>2} / {n_safe} ({single_clean_safeg['SAFE']['UNKNOWN'] / n_safe * 100:5.1f}%)"
    )
    print(
        f"{'':<35} | {'Confusion (N=32)':<15} | {single_clean_safeg['CONFUSION']['CONFIRMED']:>2} / {n_conf} ({single_clean_safeg['CONFUSION']['CONFIRMED'] / n_conf * 100:5.1f}%)   | {single_clean_safeg['CONFUSION']['REVIEW_REQUIRED']:>2} / {n_conf} ({single_clean_safeg['CONFUSION']['REVIEW_REQUIRED'] / n_conf * 100:5.1f}%)   | {single_clean_safeg['CONFUSION']['UNKNOWN']:>2} / {n_conf} ({single_clean_safeg['CONFUSION']['UNKNOWN'] / n_conf * 100:5.1f}%)"
    )
    print("-" * 110)
    print(
        f"{'Degraded Probe (Safeguarded 5N)':<35} | {'Safe (N=5)':<15} | {single_deg_safeg['SAFE']['CONFIRMED']:>2} / {n_safe} ({single_deg_safeg['SAFE']['CONFIRMED'] / n_safe * 100:5.1f}%)   | {single_deg_safeg['SAFE']['REVIEW_REQUIRED']:>2} / {n_safe} ({single_deg_safeg['SAFE']['REVIEW_REQUIRED'] / n_safe * 100:5.1f}%)   | {single_deg_safeg['SAFE']['UNKNOWN']:>2} / {n_safe} ({single_deg_safeg['SAFE']['UNKNOWN'] / n_safe * 100:5.1f}%)"
    )
    print(
        f"{'':<35} | {'Confusion (N=32)':<15} | {single_deg_safeg['CONFUSION']['CONFIRMED']:>2} / {n_conf} ({single_deg_safeg['CONFUSION']['CONFIRMED'] / n_conf * 100:5.1f}%)   | {single_deg_safeg['CONFUSION']['REVIEW_REQUIRED']:>2} / {n_conf} ({single_deg_safeg['CONFUSION']['REVIEW_REQUIRED'] / n_conf * 100:5.1f}%)   | {single_deg_safeg['CONFUSION']['UNKNOWN']:>2} / {n_conf} ({single_deg_safeg['CONFUSION']['UNKNOWN'] / n_conf * 100:5.1f}%)"
    )
    ...
    print(
        f"{'Pipeline Stage':<35} | {'Subgroup':<15} | {'CONFIRMED':<18} | {'REVIEW_REQUIRED':<18} | {'UNKNOWN (Lost)'}"
    )
    print("-" * 110)
    print(
        f"{'Single-Frame Degraded (Step 5N)':<35} | {'Safe (N=5)':<15} | {single_deg_safeg['SAFE']['CONFIRMED']:>2} / {n_safe} ({single_deg_safeg['SAFE']['CONFIRMED'] / n_safe * 100:5.1f}%)   | {single_deg_safeg['SAFE']['REVIEW_REQUIRED']:>2} / {n_safe} ({single_deg_safeg['SAFE']['REVIEW_REQUIRED'] / n_safe * 100:5.1f}%)   | {single_deg_safeg['SAFE']['UNKNOWN']:>2} / {n_safe} ({single_deg_safeg['SAFE']['UNKNOWN'] / n_safe * 100:5.1f}%)"
    )
    print(
        f"{'':<35} | {'Confusion (N=32)':<15} | {single_deg_safeg['CONFUSION']['CONFIRMED']:>2} / {n_conf} ({single_deg_safeg['CONFUSION']['CONFIRMED'] / n_conf * 100:5.1f}%)   | {single_deg_safeg['CONFUSION']['REVIEW_REQUIRED']:>2} / {n_conf} ({single_deg_safeg['CONFUSION']['REVIEW_REQUIRED'] / n_conf * 100:5.1f}%)   | {single_deg_safeg['CONFUSION']['UNKNOWN']:>2} / {n_conf} ({single_deg_safeg['CONFUSION']['UNKNOWN'] / n_conf * 100:5.1f}%)"
    )
    ...
    print(
        f"{'':<35} | {'Confusion (N=32)':<15} | {track_deg_safeg['CONFUSION']['CONFIRMED']:>2} / {n_conf} ({track_deg_safeg['CONFUSION']['CONFIRMED'] / n_conf * 100:5.1f}%)   | {track_deg_safeg['CONFUSION']['REVIEW_REQUIRED']:>2} / {n_conf} ({track_deg_safeg['CONFUSION']['REVIEW_REQUIRED'] / n_conf * 100:5.1f}%)   | {track_deg_safeg['CONFUSION']['UNKNOWN']:>2} / {n_conf} ({track_deg_safeg['CONFUSION']['UNKNOWN'] / n_conf * 100:5.1f}%)"
    )

    print("\n--- FAR TEST AUDIT SPECIFICATIONS ---")
    print(
        f"  - Total Pairwise Impostor Comparisons Evaluated under Real Pixel Degradation : {total_cross_comparisons:,}"
    )
    print(f"  - Total False Acceptances Across Any Cross-Identity Probe                     : {total_false_accepts}")
    print(
        f"  - Empirical Cross-Subject FAR                                                 : 0.00% (0 / {total_cross_comparisons:,})"
    )
