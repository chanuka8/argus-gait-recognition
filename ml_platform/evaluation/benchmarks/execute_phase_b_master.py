...
def main():
    print("=" * 100)
    print("ARGUS AI COMPREHENSIVE DUAL-MODAL BIOMETRIC EVALUATION (PHASE B MASTER)")
    print("=" * 100)
    ...
    for i in range(N):
        ...
        best_g_idx = int(np.argmax(sim_matrix_gait[i]))
        best_a_idx = int(np.argmax(sim_matrix_app[i]))

        g_score = float(sim_matrix_gait[i, best_g_idx])
        g_id = loo_gallery_labels[best_g_idx]
        ...
        dec = fusion_engine.decide_identity(
            gait_identity=g_id,
            gait_score=g_score,
            appearance_identity=a_id,
            appearance_score=a_score,
            gait_threshold=0.89,
            appearance_threshold=0.72,
        )

        if dec["decision"] == "CONFIRMED":
            if dec["final_identity"] == q_lbl:
                gated_correct["fused"] += 1
            else:
                gated_false["fused"] += 1
        else:
    ...
    frr_app = gated_unknown["app"] / N
    tar_app = gated_correct["app"] / N
    far_app = gated_false["app"] / N

    frr_fused = gated_unknown["fused"] / N
    tar_fused = gated_correct["fused"] / N
    far_fused = gated_false["fused"] / N
    ...
    for w_g in weight_steps:
        ...
        unknowns = 0
        for i in range(N):
            best_idx = int(np.argmax(sweep_sim_matrix[i]))
            best_score = sweep_sim_matrix[i, best_idx]
            best_id = loo_gallery_labels[best_idx]
            if best_score >= th_zero_far:
                if best_id == query_labels[i]:
                    hits += 1
            else:
                unknowns += 1
        ...
        for i in range(N):
            ...
            if loo_gallery_labels[order[0]] == query_labels[i]:
                correct_margins.append(float(sims[order[0]] - sims[order[1]]))
        ...
        rec = {
            ...
            "gated_tar": round(gated_tar, 2),
            "gated_frr": round(gated_frr, 2),
    ...
    same_f = [0.30 * sg + 0.70 * sa for sg, sa in zip(same_g, same_a)]
    diff_f = [0.30 * dg + 0.70 * da for dg, da in zip(diff_g, diff_a)]
    ...
    for p in probe_records:
        ...
        if v_angle in view_cond_counts[c_type]:
            view_cond_counts[c_type][v_angle]["total"] += 1
            if is_correct:
                view_cond_counts[c_type][v_angle]["correct"] += 1

    print("\nCASIA-B Disjoint Condition Accuracy (50 Test Subjects, Gallery: 90 deg NM-01..04):")
    for c_type in ["NM", "BG", "CL"]:
        c_res = cond_counts[c_type]
        c_acc = c_res["correct"] / max(c_res["total"], 1) * 100
        print(f"  - {c_type:15}: {c_acc:>6.2f}% ({c_res['correct']}/{c_res['total']} probes)")

    print("\n--- 11-VIEW ANGLE ACCURACY MATRIX (Gallery: 90 deg) ---")
    print(f"{'Condition':<12} | " + " | ".join([f"{a:>5}deg" for a in all_angles]) + " | {'Mean':>6}")
    print("-" * 115)
    view_acc_matrix = {}
    for c_type in ["NM", "BG", "CL"]:
        row_vals = []
        for a in all_angles:
            res = view_cond_counts[c_type][a]
            acc = res["correct"] / max(res["total"], 1) * 100
            row_vals.append(acc)
        view_acc_matrix[c_type] = {a: round(v, 2) for a, v in zip(all_angles, row_vals)}
    ...
    for sid in unseen_casia_subjects:
        ...
        for f in gei_files:
            intruder_gei_embs.append(gait_extractor.extract(f))
    ...
    bygait_gpu = ByGaitLight(part_bins=1).to(device if torch.cuda.is_available() else "cpu").eval()
    bygait_cpu = ByGaitLight(part_bins=1).to("cpu").eval()
    ...
    size_app_mb = Path("ml_platform/models/model_store/weights/osnet_x0_25.pth").stat().st_size / (1024 * 1024)
    ...
    gated_tars = [r["gated_tar"] for r in sweep_records]
    gated_frrs = [r["gated_frr"] for r in sweep_records]
    aucs = [r["auc"] * 100 for r in sweep_records]

    plt.plot(w_gaits, rank1s, "b-o", linewidth=2, label="Rank-1 Accuracy (%)")
    plt.plot(w_gaits, gated_tars, "g-^", linewidth=2, label="Gated Accept Rate / TAR (%)")
    plt.plot(w_gaits, gated_frrs, "r--v", linewidth=2, label="False Rejection Rate / FRR (%)")
    plt.plot(w_gaits, aucs, "m-.s", linewidth=1.5, label="ROC-AUC (x100)")
    plt.axvline(x=0.30, color="k", linestyle=":", label="Configured Split (0.30 Gait / 0.70 App)")
    plt.title("Fusion Weight Sweep Ablation: Identification & Verification Trade-off", fontsize=13, fontweight="bold")
    ...
    report_dict = {
        ...
        "reconciliation_closed_vs_gated": {
            "gait_only": {
                "rank1_closed_pct": round(rank1_gait_acc * 100, 2),
                "gated_tar_pct": round(tar_gait * 100, 2),
                "gated_frr_pct": round(frr_gait * 100, 2),
                "gated_far_pct": round(far_gait * 100, 2),
                "gated_unknown_count": gated_unknown["gait"],
            },
            "appearance_only": {
                "rank1_closed_pct": round(rank1_app_acc * 100, 2),
                "gated_tar_pct": round(tar_app * 100, 2),
                "gated_frr_pct": round(frr_app * 100, 2),
                "gated_far_pct": round(far_app * 100, 2),
                "gated_unknown_count": gated_unknown["app"],
            },
            "dual_modal_fused": {
                "rank1_closed_pct": round(rank1_fused_acc * 100, 2),
                "gated_tar_pct": round(tar_fused * 100, 2),
        ...
        "nested_cross_validation": {
            ...
            "out_of_sample_gated_tar_pct": round(float(np.mean(cv_test_recalls) * 100), 2),
            "out_of_sample_gated_frr_pct": round(float(np.mean(cv_test_frr_scores) * 100), 2),
            "out_of_sample_gated_far_pct": round(float(np.mean(cv_test_far_scores) * 100), 2),
        },
