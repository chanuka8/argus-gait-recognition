...
from ml_platform.evaluation.metrics import EvaluationMetrics
from ml_platform.evaluation.roc import ROCAnalyzer


class SweepEvaluator(SplitEvaluator):
    def _build_split_data(self):
        store = VectorStore(gallery_dir="ml_platform/models/galleries/gallery")
        gallery = store.load()
        if gallery is None:
