...
def _to_rel(path: Union[str, Path]) -> str:
...
def _execute_doctor_checks(json_path: str, md_path: str) -> Tuple[int, dict]:
    ...
    checks.append({
        ...
        "details": f"PyTorch checkpoint at '{_to_rel(ckpt_path)}' (" + ("found" if ckpt_exists else "missing") + ")",
    })
    if not ckpt_exists:
    ...
    md_content = f"""# Doctor System Health Report
