...
| [test_detector.py](test_detector.py) | Unit tests for YOLO person detector bounding box outputs |
| [test_gei_stream.py](test_gei_stream.py) | Tests rolling window silhouette accumulation and GEI generation |
| [test_leakage_prevention.py](test_leakage_prevention.py) | Asserts zero identity overlap between evaluation dataset partitions |
| [test_logging.py](test_logging.py) | Tests multi-channel rotating log file initialization and logging output |
| [test_multi_camera.py](test_multi_camera.py) | Tests multi-camera recognition pipeline and cross-camera evidence fusion |
| [test_phase4_streaming.py](test_phase4_streaming.py) | Tests stream engine buffering, queueing, and frame dropper behavior |
| [test_phase5_cctv.py](test_phase5_cctv.py) | Tests CCTV camera manager and worker thread pool operations |
| [test_phase6_intelligence.py](test_phase6_intelligence.py) | Tests intelligence modules: open-set recognition, decision engine, and score normalizers |
| [test_rtsp_credentials.py](test_rtsp_credentials.py) | Tests RTSP stream credential parsing and secure encryption storage |
| [test_silhouette.py](test_silhouette.py) | Tests silhouette extraction, Otsu thresholding, morphology, and contour filtering |
| [test_tracker.py](test_tracker.py) | Tests ByteTrack multi-object tracking and track ID assignment |
| [test_watchdog.py](test_watchdog.py) | Tests system watchdog health checks and component auto-restart logic |
| [test_watchlist_integration.py](test_watchlist_integration.py) | Tests real-time missing person watchlist workflow and match event processing |
