...
def test_05_unauthorized_role_camera_start_rejected_with_403():
    """INVARIANT 5: Unauthorized session without camera permissions -> 403 Forbidden."""
    store = get_session_store()
    session = store.create_session(operator_id="guest_01", username="guest_01", role="guest")
    ...
    with TestClient(app) as client:
        ...
        assert "Operation requires camera start privileges" in resp.json()["detail"]
