...
def test_1_live_cctv_to_collector_and_persistence(target_env):
    ...
    # Mock components for live pipeline
    ...
    # Feed frames to trigger appearance crop and gait GEI
    ...
    assert len(app_obs.observation_date) == 10  # YYYY-MM-DD
    ...
    # Reload in fresh collector instance
...
def test_2_deduplication_and_rate_limiting(target_env):
    ...
    # 2nd call with identical vector 0.05s later: deduplicated
...
def test_3_ground_truth_verification_and_scheduling(target_env):
    ...
    # Record 6 observations across 2 identities on date 2026-08-29
    obs_ids = []
    for i in range(3):
    ...
    # Idempotency check: duplicate call does not duplicate job
...
def test_4_model_registry_validation_and_rollback(target_env):
    ...
    # Unvalidated promotion must be rejected
