...
def test_appearance_embedding_normalization_and_gating():
    ...
    # Fake crop (BGR image 100x50x3)
    crop = np.full((100, 50, 3), 128, dtype=np.uint8)

    # Extract for track 1 at frame 0
    ...
    # Frame 2 (< update_interval=5): should return cached without error
    emb2 = extractor.extract(crop, track_id=1, frame_index=2)
    if emb1 is not None:
        assert np.allclose(emb1, emb2)

    # Track unreliable -> should return cached
