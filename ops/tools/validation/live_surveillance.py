...
from app.intelligence.tracking.concurrent_track_manager import ConcurrentTrackManager
from app.pipeline.detection.detection_validator import DetectionValidator
...
from app.utils.display_renderer import DetectionDisplayRenderer
...
def run_live_validation(num_frames: int = 45) -> dict[str, Any]:
    ...
    print(
        f"[REAL PHYSICAL VALIDATION] Active Resolution: {actual_w}x{actual_h} @ {actual_fps:.1f} FPS (Backend: CAP_DSHOW)"
    )
    ...
    for frame_idx in range(num_frames):
        ...
        if (frame_idx + 1) % 15 == 0 or frame_idx == num_frames - 1:
            print(
                f"  -> Frame {frame_idx + 1}/{num_frames}: {len(raw_detections)} detections, {len(tracked_objects)} tracks, RSS: {metrics['memory_rss_mb'][-1]:.1f} MB"
            )
    ...
    print(
        f"[REAL PHYSICAL VALIDATION] Overlays: RED={metrics['red_overlays']}, GREEN={metrics['green_overlays']}, YELLOW={metrics['yellow_overlays']}"
    )
        ...
        json.dump(
            {
                ...
                "performance": {
                    ...
                    "avg_detection_ms": round(float(np.mean(metrics["detection_times_ms"])), 2)
                    if metrics["detection_times_ms"]
                    else 0.0,
                    "avg_tracking_ms": round(float(np.mean(metrics["tracking_times_ms"])), 2)
                    if metrics["tracking_times_ms"]
                    else 0.0,
                    "avg_appearance_ms": round(float(np.mean(metrics["appearance_times_ms"])), 2)
                    if metrics["appearance_times_ms"]
                    else 0.0,
                    "avg_render_ms": round(float(np.mean(metrics["render_times_ms"])), 2)
                    if metrics["render_times_ms"]
                    else 0.0,
